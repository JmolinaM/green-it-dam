...
Un software sostenible debe impactar positivamente en ambos factores (E y CI), además de considerar las emisiones asociadas al hardware que lo soporta. La figura~\ref{fig:gse-principios} ilustra cómo los tres principios principales de la GSE reducen la huella de carbono y cómo se aplican en las prácticas de este TFM:

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  col1/.style={rectangle, rounded corners=4pt, minimum width=3.2cm, minimum height=1.0cm,
               draw=green!60!black, fill=green!15, align=center, font=\small\bfseries},
  col2/.style={rectangle, rounded corners=4pt, minimum width=5.6cm, minimum height=1.0cm,
               draw=gray!40, fill=gray!6, align=center, font=\small},
  col3/.style={rectangle, rounded corners=4pt, minimum width=2.4cm, minimum height=1.0cm,
               draw=practica1!80!black, fill=practica1!30, align=center, font=\small\bfseries},
  hdr/.style={font=\small\bfseries\itshape, text=gray!60}
]
  % Headers
  \node[hdr] (h1) at (0,0)     {Principio GSE};
  \node[hdr] (h2) at (4.5,0)   {Qué reduce};
  \node[hdr] (h3) at (9.3,0)   {Práctica};

  % Row 1 — P.I Eficiencia
  \node[col1] (r1a) at (0,-1.3)   {P.\,I\\[-2pt]\footnotesize Eficiencia energética};
  \node[col2] (r1b) at (4.5,-1.3) {CPU · memoria · red:\\[-2pt]\footnotesize menos energía para el mismo resultado};
  \node[col3] (r1c) at (9.3,-1.3) {P1 · P3};

  % Row 2 — P.II Carbon-Aware
  \node[col1, fill=practica4!20, draw=practica4!70!black]
        (r2a) at (0,-2.8)   {P.\,II\\[-2pt]\footnotesize Carbon-Aware};
  \node[col2] (r2b) at (4.5,-2.8) {Intensidad de carbono (CI):\\[-2pt]\footnotesize ejecutar cuando la red eléctrica es más verde};
  \node[col3, fill=practica4!30, draw=practica4!80!black]
        (r2c) at (9.3,-2.8) {P4};

  % Row 3 — P.III Hardware
  \node[col1, fill=practica3!30, draw=practica3!70!black]
        (r3a) at (0,-4.3)   {P.\,III\\[-2pt]\footnotesize Hardware eficiente};
  \node[col2] (r3b) at (4.5,-4.3) {Carbono incorporado:\\[-2pt]\footnotesize menos RAM/CPU prolonga la vida del equipo};
  \node[col3, fill=practica3!40, draw=practica3!80!black]
        (r3c) at (9.3,-4.3) {P2};

  % Separator lines
  \draw[gray!25, line width=0.4pt] (-1.8,-0.55) -- (10.8,-0.55);
  \draw[gray!20, line width=0.3pt] (-1.8,-2.05) -- (10.8,-2.05);
  \draw[gray!20, line width=0.3pt] (-1.8,-3.55) -- (10.8,-3.55);
\end{tikzpicture}
\caption{Principios de la Green Software Engineering y su aplicación en las prácticas de este TFM}
...
La conexión entre el marco teórico y las prácticas es directa: \textbf{P0} introduce la magnitud del problema (la cadena código\,→\,CO\textsubscript{2} de la figura~\ref{fig:cadena-co2}); \textbf{P1} y \textbf{P3} entrenan el Principio\,I midiendo y optimizando algoritmos e E/S; \textbf{P2} lleva el Principio\,III al análisis de memoria de objetos; y \textbf{P4} cierra el ciclo aplicando el Principio\,II en un sistema real con datos de la red eléctrica.
