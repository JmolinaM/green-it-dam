...
Este capítulo revisa la literatura científica en torno a tres bloques: los fundamentos pedagógicos de una educación que no se limita a la concienciación sino que promueve la acción real (el apartado 2.1), el impacto ambiental del software y las TIC (el apartado 2.2), y el estado actual de la integración de Green IT en la educación informática (el apartado 2.3).
...
Un software sostenible debe impactar positivamente en ambos factores (E y CI), además de considerar las emisiones asociadas al hardware que lo soporta. La figura~\ref{fig:gse-principios} ilustra los tres principios de la GSE:

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  card/.style={rectangle, rounded corners=8pt, minimum width=3.8cm,
               minimum height=5.6cm, draw=green!55!black, fill=green!8,
               text width=3.4cm, align=center},
  ctitle/.style={font=\small\bfseries, align=center},
  cdesc/.style={font=\footnotesize, align=center, text width=3.0cm},
  bar/.style={rectangle, draw=none, fill=green!45!black},
  slot/.style={rectangle, rounded corners=2pt, minimum width=0.45cm,
               minimum height=0.7cm, draw=none},
  device/.style={rectangle, rounded corners=3pt, draw=gray!60,
                 fill=gray!15, minimum width=1.0cm, minimum height=0.7cm}
]

%% ---- CARD 1: Eficiencia energética ----
\node[card, fill=green!8, draw=green!55!black] (c1) at (0,0) {};

% Mini bar chart: antes (tall, red) vs después (short, green)
\node[bar, fill=red!40, minimum width=0.55cm, minimum height=1.6cm]
  at (-0.5, 1.6) {};
\node[bar, fill=green!50!black, minimum width=0.55cm, minimum height=0.7cm]
  at (0.5, 1.15) {};
\node[font=\tiny, text=gray!70] at (-0.5, 0.65) {antes};
\node[font=\tiny, text=gray!70] at (0.5, 0.65)  {después};
\draw[->, thick, green!50!black] (-0.1, 1.3) -- (0.2, 0.9);

\node[ctitle] at (0, 0.0) {Principio I\\Eficiencia energética};
\draw[gray!30] (-1.6,-0.25) -- (1.6,-0.25);
\node[cdesc] at (0,-1.2)
  {El mismo resultado con menos energía:\\
   optimizar algoritmos,\\
   memoria y transferencia de datos.\\[4pt]
   \textit{Reducir E en la fórmula}\\
   CO\textsubscript{2} = \textbf{E} $\times$ CI};

%% ---- CARD 2: Carbon-Aware ----
\node[card, fill=practica4!12, draw=practica4!70!black] (c2) at (4.4,0) {};

% Mini timeline: slots grey/grey/green/green showing best moment
\foreach \x/\col in {-0.9/gray!25, -0.45/gray!25, 0.0/green!40, 0.45/green!40, 0.9/gray!25} {
  \node[slot, fill=\col] at (\x+4.4, 1.5) {};
}
\node[font=\tiny, text=gray!60] at (4.4, 0.85) {hora del día →};
\draw[->, thick, practica4!70!black] (4.4-0.22, 1.1) -- (4.4+0.22, 1.1)
  node[midway, above, font=\tiny, text=practica4!80!black] {mejor momento};

\node[ctitle] at (4.4, 0.0) {Principio II\\Carbon-Aware};
\draw[gray!30] (4.4-1.6,-0.25) -- (4.4+1.6,-0.25);
\node[cdesc] at (4.4,-1.2)
  {La misma energía emite distinto CO\textsubscript{2} según cuándo se consume.\\[4pt]
   Ejecutar tareas cuando la red\\
   eléctrica usa más\\
   renovables.\\[4pt]
   \textit{Reducir CI en la fórmula}\\
   CO\textsubscript{2} = E $\times$ \textbf{CI}};

%% ---- CARD 3: Hardware eficiente ----
\node[card, fill=practica3!15, draw=practica3!70!black] (c3) at (8.8,0) {};

% Device with long lifespan arrow
\node[device] at (8.8, 1.6) {\tiny equipo};
\draw[->, thick, practica3!60!black, rounded corners=4pt]
  (8.8-0.7, 1.15) -- (8.8-0.7, 0.85) -- (8.8+0.7, 0.85) -- (8.8+0.7, 1.15);
\node[font=\tiny, text=gray!60] at (8.8, 0.65) {vida útil extendida};

\node[ctitle] at (8.8, 0.0) {Principio III\\Hardware eficiente};
\draw[gray!30] (8.8-1.6,-0.25) -- (8.8+1.6,-0.25);
\node[cdesc] at (8.8,-1.2)
  {Fabricar un dispositivo emite más CO\textsubscript{2} que usarlo.\\[4pt]
   Software ligero (menos RAM/CPU) prolonga la vida del hardware\\
   y reduce el carbono\\
   incorporado.\\[4pt]
   \textit{Reducir M en SCI = (O+M)/R}};

\end{tikzpicture}
\caption{Los tres principios de la Green Software Engineering: qué reduce cada uno y por qué}
...
La tabla~\ref{tab:gse-principios} sintetiza los tres principios con mayor relevancia para la formación en el ciclo de DAM y su aplicación directa en este TFM:
...
\toprule
\textbf{Principio GSE} & \textbf{Qué significa} & \textbf{Cómo se trabaja en las prácticas} \\
\midrule
\textbf{I — Eficiencia energética} &
  Hacer el mismo trabajo computacional consumiendo menos energía, optimizando algoritmos, memoria y transferencia de datos &
  \textbf{P1}: comparativa de algoritmos con CodeCarbon · \textbf{P3}: streaming vs.\ carga completa \\
\addlinespace
\textbf{II — Carbon-Aware} &
  Adaptar la ejecución al momento o región con mayor proporción de energía renovable en la red &
  \textbf{P4}: cola de tareas con Electricity Maps API \\
\addlinespace
\textbf{III — Hardware eficiente} &
  Minimizar los recursos de hardware necesarios para prolongar la vida útil del dispositivo &
  \textbf{P2}: perfilado de memoria con \texttt{tracemalloc}; relación $O(n)$\,→\,consumo \\
\bottomrule
\end{tabular}
\caption{Principios GSE y su aplicación en las prácticas de este TFM}
\label{tab:gse-principios}
\end{table}
