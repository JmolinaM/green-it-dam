...
Un software sostenible debe impactar positivamente en ambos factores (E y CI), además de considerar las emisiones asociadas al hardware que lo soporta. La figura~\ref{fig:cadena-co2} muestra la cadena causal y los puntos de intervención de la GSE:

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
  node distance=1.6cm and 2.0cm,
  box/.style={rectangle, rounded corners=5pt, draw=green!50!black,
              fill=green!8, text width=2.0cm, align=center,
              font=\small\bfseries, minimum height=1.1cm},
  gbox/.style={rectangle, rounded corners=3pt, draw=gray!50,
               fill=gray!7, text width=2.2cm, align=center,
               font=\footnotesize\itshape, minimum height=0.9cm},
  arr/.style={-{Stealth}, thick, green!50!black},
  darr/.style={-{Stealth}, dashed, gray!60}
]
  % Main chain
  \node[box] (code)   {Código\\ejecutado};
  \node[box, right=of code]   (cpu)    {CPU · Mem.\\E/S · Red};
  \node[box, right=of cpu]    (energy) {Energía\\consumida (E)};
  \node[box, right=of energy] (co2)    {CO\textsubscript{2}\\emitido};

  \draw[arr] (code)   -- (cpu);
  \draw[arr] (cpu)    -- (energy);
  \draw[arr] (energy) -- (co2) node[midway, above, font=\footnotesize, black] {$\times$ CI};

  % GSE principles below
  \node[gbox, below=1.0cm of code]   (p1a) {P.\,I\\Algoritmos eficientes};
  \node[gbox, below=1.0cm of cpu]    (p1b) {P.\,I\\Memoria · E/S};
  \node[gbox, below=1.0cm of energy] (p2)  {P.\,II\\Carbon-Aware};
  \node[gbox, below=1.0cm of co2]    (p3)  {P.\,III\\Hardware eficiente};

  \draw[darr] (p1a.north) -- (code.south);
  \draw[darr] (p1b.north) -- (cpu.south);
  \draw[darr] (p2.north)  -- (energy.south);
  \draw[darr] (p3.north)  -- (co2.south);
\end{tikzpicture}
\caption{Cadena causal código\,→\,CO\textsubscript{2} y puntos de intervención de los principios GSE}
\label{fig:cadena-co2}
\end{figure}
