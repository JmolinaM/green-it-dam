...
\begin{tikzpicture}[
  card/.style={rectangle, rounded corners=8pt, minimum width=3.8cm,
               minimum height=5.8cm, draw=green!55!black, fill=green!8,
  ...
  bar/.style={rectangle, draw=none},
  slot/.style={rectangle, rounded corners=2pt, minimum width=0.44cm,
...
% Bar chart: antes (red/tall) vs después (green/short)
\node[bar, fill=red!40,        minimum width=0.55cm, minimum height=1.6cm] at (-0.5, 1.6) {};
\node[bar, fill=green!55!black, minimum width=0.55cm, minimum height=0.7cm] at ( 0.5, 1.15) {};
\node[font=\tiny, text=gray!60] at (-0.5, 0.65) {antes};
\node[font=\tiny, text=gray!60] at ( 0.5, 0.65) {despu\'{e}s};
\draw[->, thick, green!55!black] (-0.1, 1.3) -- (0.2, 0.9);
\node[ctitle] at (0, 0.1) {Principio I\\Eficiencia energ\'{e}tica};
\draw[gray!30] (-1.7,-0.25) -- (1.7,-0.25);
\node[cdesc] at (0,-1.2)
  {El mismo resultado con menos energ\'{i}a:\\
   optimizar algoritmos, memoria\\
   y transferencia de datos.\\[4pt]
   \textit{Reducir \textbf{E} en:}\\
   CO\textsubscript{2} $=$ \textbf{E} $\times$ CI};

%% ---- CARD 2: Carbon-Aware ----
\node[card, fill=practica4!12, draw=practica4!70!black] (c2) at (4.4,0) {};
% Time slots: grey grey GREEN GREEN grey
\node[slot, fill=gray!25]   at (3.74, 1.5) {};
\node[slot, fill=gray!25]   at (4.18, 1.5) {};
\node[slot, fill=green!45]  at (4.62, 1.5) {};
\node[slot, fill=green!45]  at (5.06, 1.5) {};
\node[slot, fill=gray!25]   at (5.50, 1.5) {};
\node[font=\tiny, text=gray!60] at (4.4, 0.85) {hora del d\'{i}a $\rightarrow$};
\node[font=\tiny, text=practica4!80!black] at (4.62, 1.1) {\textbf{ahora}};
\node[ctitle] at (4.4, 0.1) {Principio II\\Carbon-Aware};
\draw[gray!30] (2.8,-0.25) -- (6.0,-0.25);
\node[cdesc] at (4.4,-1.2)
  {La misma energ\'{i}a emite distinto CO\textsubscript{2} seg\'{u}n cu\'{a}ndo.\\[4pt]
   Ejecutar tareas cuando la red\\
   el\'{e}ctrica usa m\'{a}s renovables.\\[4pt]
   \textit{Reducir \textbf{CI} en:}\\
   CO\textsubscript{2} $=$ E $\times$ \textbf{CI}};

%% ---- CARD 3: Hardware eficiente ----
\node[card, fill=practica3!15, draw=practica3!70!black] (c3) at (8.8,0) {};
% Device box with U-arrow below (= extended life)
\node[device] (dev) at (8.8, 1.7) {\scriptsize equipo};
\draw[->, thick, practica3!60!black, rounded corners=3pt]
  (8.1, 1.3) -- (8.1, 1.0) -- (9.5, 1.0) -- (9.5, 1.3);
\node[font=\tiny, text=gray!60] at (8.8, 0.7) {vida \'{u}til extendida};
\node[ctitle] at (8.8, 0.1) {Principio III\\Hardware eficiente};
\draw[gray!30] (7.2,-0.25) -- (10.4,-0.25);
\node[cdesc] at (8.8,-1.2)
  {Fabricar hardware emite m\'{a}s CO\textsubscript{2} que usarlo.\\[4pt]
   Software ligero (menos RAM/CPU)\\
   prolonga la vida del equipo\\
   y reduce el carbono incorporado.\\[4pt]
   \textit{Reducir \textbf{M} en SCI $=$ (O$+$M)/R}};

\end{tikzpicture}
\caption{Los tres principios de la Green Software Engineering}
