...
\textbf{Objetivo:} Construir el modelo mental de por qué el \textit{software} tiene huella de carbono y verificarlo con datos reales.
...
\textbf{(10 min) Puesta en común y ranking} Cada alumno anota en la pizarra su \% de ahorro total. El \textit{ranking} compara porcentajes de reducción (no valores absolutos, que dependen del \textit{hardware}). Cuando toda la clase ve ahorros superiores al 99 \%, la norma social consolida que esa mejora es posible y esperable \cite{cialdini2001influence}.
...
\fichapractica{P2}{El servidor que no para de crecer}
  ...
  {P.III — \textit{Hardware} eficiente · P.IV — Medición SCI}
...
Como se introduce en el apartado~\ref{sec:carbono-incorporado} (\textit{carbono operacional vs.\ incorporado}), fabricar un dispositivo ya genera una \textbf{huella de carbono incorporada} que se diluye cuantos más años dura el equipo. Cuidar el \textit{hardware} pasa, en gran medida, por un \textit{software} que no lo fuerce más allá de sus límites: un programa con fugas de memoria obliga a reinicios frecuentes y acorta la vida útil del servidor.
...
Los equipos trabajan de forma autónoma. Si un equipo lleva más de 10 minutos sin avanzar, el Analista de datos puede pedir al docente \textbf{una sola pregunta de orientación} (p.~ej., \textit{``¿Qué atributo del objeto crece cada vez que llega un dato nuevo?''} o \textit{``Si ejecutáis el script para n=100 y para n=1000, ¿qué línea de la traza cambia más?''}).
...
\item \textbf{Estrategia B:} \texttt{collections.deque(maxlen=X)} para \texttt{\_historial} (\textit{buffer} circular)
\item \textbf{Estrategia C:} Eliminación del \texttt{\_registro\_config} completo
\item \textbf{Estrategia D:} Combinación de varias
\end{itemize}


La tarjeta incluye: solución elegida / por qué resuelve el problema / \textit{trade-off} asumido / métrica de éxito cuantificada.
...
Cada equipo presenta en \textbf{5 minutos exactos} con estructura obligatoria: (1) diagnóstico con dato, (2) solución y código refactorizado, (3) datos antes/después y extrapolación, (4) \textit{trade-off} asumido. El docente anota la tabla comparativa en la pizarra.
...
\fichapractica{P3}{Datos que cuestan energía}
  ...
  {Se ha comparado el consumo energético de diferentes estrategias de E/S (carga completa vs.\ \textit{streaming}), seleccionando la más eficiente energéticamente}
  {ABP individual}
  {3 sesiones $\times$ 50 min}
  {P.I — Eficiencia energética · P.II — \textit{Carbon-aware}}
...
\textbf{(30 min) Fase 3: Conceptualización Abstracta} El alumnado formaliza: (a) la diferencia entre carga completa y \textit{streaming} como $O(n)$ vs. $O(k)$, donde k es el tamaño del \textit{chunk}; (b) por qué el \textit{streaming} es el estándar en sistemas de grandes datos; (c) la relación entre consumo de RAM y coste energético en servidores \textit{cloud} (precio por GB·hora en AWS/Azure).
...
\textbf{(50 min) Fase 4: Experimentación Activa} El alumnado extiende la solución de \textit{streaming} para calcular en un único paso cuatro estadísticas (media, desviación típica, mínimo y máximo). Opcionalmente explora la lectura del CSV comprimido con gzip. Redacta el informe comparando las tres variantes: carga completa, \textit{streaming} y \textit{streaming} con compresión.
...
\fichapractica{P4}{La cola verde}
  ...
  {P.I — Eficiencia energética · P.II — \textit{Carbon-aware} · P.III — \textit{Hardware} eficiente · P.IV — Medición SCI}
...
\textbf{(50 min) Fase 3: Conceptualización Abstracta} Los grupos implementan la clase \texttt{ColaVerde} con dos métodos principales: \texttt{encolar(tarea, urgente, umbral\_co2)} y \texttt{ejecutar\_pendientes(intensidad\_actual)}. Las tareas urgentes se ejecutan inmediatamente; las diferibles, solo si la intensidad actual es inferior al umbral. Se introduce \texttt{heapq} como estructura interna para la cola de prioridad. El docente circula y orienta en los bloqueos de implementación.
...
\multirow{2}{2.8cm}{\raggedright Análisis de rendimiento}
  ...
  & \texttt{pandas}
  & Procesado de datos en \textit{streaming} vs.\ carga completa (P3) \\
\midrule
\multirow{2}{*}{Pruebas}
  & \texttt{pytest} \newline \texttt{pip install pytest}
  & Tests unitarios y de integración de la ColaVerde (P4) \\
  & \texttt{heapq} (stdlib Python)
  & Implementación de la cola de prioridad \textit{carbon-aware} (P4) \\
\midrule
